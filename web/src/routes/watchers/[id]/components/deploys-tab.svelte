...
<div class="mb-3 flex items-center justify-between gap-2">
	...
	<div class="flex items-center gap-2">
		...
		<Button.Root
			...
			onclick={() => onPageChange(deployPage - 1)}
		>
			Prev
		</Button.Root>
		<Button.Root
		...
		<Table.Root>
			...
			<Table.Body>
				{#each deploys as d (d.id)}
					{@const Icon = getDeployIcon(d.status)}
							...
							<div class="flex items-center justify-end gap-2">
								{#if d.github_deployment_id > 0}
									<span
										...
										class="inline-flex items-center rounded border border-muted-foreground/20 bg-muted/30 px-1 py-0.5 text-[10px] text-muted-foreground/60"
									>
								...
								{#if d.logs}
									<a
										href={resolve(`/watchers/${watcherId}/logs/${d.id}`)}
